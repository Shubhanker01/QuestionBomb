...
import { useMockReview } from '@/hooks/useMockReview';
...
export default function TestReview() {
  ...
  if (isLoading) {
    return (
      <div>
        Please wait... Loading review data
      </div>
    )
  }
  if (error) {
      ...
      <header className="border-b bg-card px-6 py-3.5 flex items-center justify-between sticky top-0 z-20 shadow-xs">
        ...
        <div className="hidden sm:flex items-center gap-2">
          <Badge variant="outline" className="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 gap-1 py-1">
            <CheckCircle2 className="w-3.5 h-3.5" /> {stat.noOfCorrectQuestions} Correct
          </Badge>
          <Badge variant="outline" className="bg-destructive/10 text-destructive border-destructive/20 gap-1 py-1">
            <XCircle className="w-3.5 h-3.5" /> {stat.noOfIncorrectQuestions} Incorrect
          </Badge>
          <Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-amber-500/20 gap-1 py-1">
            <AlertCircle className="w-3.5 h-3.5" /> {stat.noOfUnattemptedQuestions} Skipped
          </Badge>
        </div>
      </header>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              ...
              {getStatus(activeQuestion) === 'correct' && (
                <Badge className="bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 border-emerald-500/30 gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5" /> Correct (+0.83)
                </Badge>
              )}
              {getStatus(activeQuestion) === 'incorrect' && (
                <Badge variant="destructive" className="gap-1.5">
                  <XCircle className="w-3.5 h-3.5" /> Incorrect (-0.27)
                </Badge>
              )}
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2.5">
                {activeQuestion.options.map((option: any, idx) => {
                  const isCorrectOption = idx + 1 === activeQuestion.correctAnswer;
                  const isUserChoice = idx + 1 === activeQuestion.userAnswer;
            ...
            <CardContent className="space-y-4">
              ...
              <ScrollArea className="h-64 pr-1">
                <div className="grid grid-cols-5 gap-2">
                  {filteredQuestions.map((q: QuestionReview, index) => {
                    ...
                    return (
                      <Button
                        ...
                        className={`h-9 text-xs font-bold transition-all relative ${customBg} ${isSelected ? 'ring-2 ring-primary ring-offset-2 ring-offset-background font-extrabold scale-105' : ''
                          }`}
                      >
                        {q.questionNumber}
                      </Button>
